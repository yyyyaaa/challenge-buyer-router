...
function findHighestMatchingLocation (sortedLocations, matchingLocations, cb) {
  ...
  sortedLocations = sortedLocations.filter(function (location) {
    return !sold[location]
  })
  console.log("After filter", sortedLocations)
  ...
  sortedLocations.forEach(function (location) {
    if (matchingLocations.indexOf(location) !== -1) {
      ...
      console.log("SOLD LIST ", sold)
      console.log("FOUND " + found)
    } 
  })

  handleHighest(found, cb)  
} 

function handleHighest (found, cb) {
  if (!found) {
    console.log("can't find match")
    return cb(new Error("Can't find any matching location"))
  } else {
    console.log("Sending back to cb", found)
    cb(null, found)
  }
}
